...
function TopicForm({ draft, setDraft, onSave, onCancel }) {
  return (
    <div className="bg-surface-2 rounded-xl p-4 mb-4">
      <div className="grid grid-cols-2 gap-3 mb-3">
        ...
        <div className="col-span-2">
          <p className="text-[10.5px] text-text-secondary mb-1.5">ESRS SUB-TOPIC</p>
          <select
...
function TopicRow({ topic, onUpdate, onDelete }) {
  ...
  function startEdit() {
    setEditDraft({ iroType: topic.iroType, esrsTopicId: topic.esrsTopicId, subtopic: topic.subtopic, shortTitle: topic.shortTitle, description: topic.description, actual: topic.actual, valueChain: topic.valueChain });
    setEditing(true);
    setOpen(true);
  }
  ...
  function confirmSignOff() {
    ...
    onUpdate(topic.id, { signedOffBy: signerName.trim(), signedOffAt: Date.now() });
    setSigning(false);
    setSignerName('');
  }
      ...
      {open && (
        editing ? (
          <div className="px-4 pb-4 border-t border-border-apus pt-3">
            <TopicForm draft={editDraft} setDraft={setEditDraft} onSave={saveEdit} onCancel={() => setEditing(false)} />
          </div>
        ) : (
          <div className="px-4 pb-4 text-[12px] text-text-secondary flex flex-col gap-1.5 border-t border-border-apus pt-3">
            ...
            {isSignedOff ? (
              ...
              <div className="bg-app-black rounded-lg p-3 mt-1.5">
                <p className="text-[10.5px] text-text-secondary mb-1.5">YOUR NAME (real sign-in comes later)</p>
                <div className="flex gap-2">
                  <input value={signerName} onChange={(e) => setSignerName(e.target.value)} placeholder="Full name" autoFocus className="flex-1 bg-surface-2 rounded-lg px-3 py-2 text-[12.5px] outline-none" />
                  <button onClick={confirmSignOff} disabled={!signerName.trim()} className="text-[11.5px] font-semibold rounded-lg px-3 py-2 disabled:opacity-40" style={{ background: '#5ED996', color: '#07070B' }}>Confirm</button>
...
function PerspectiveSection({ title, color, topics, types, addingType, setAddingType, draft, setDraft, onSave, onCancel, filter, onUpdateTopic, onDeleteTopic }) {
  const filtered = topics.filter((t) => types.includes(t.iroType) && (filter === 'all' || ESRS_TOPICS.find((e) => e.id === t.esrsTopicId)?.cat === filter));
  return (
    <div className="mb-8">
      ...
      {addingType && types.includes(addingType) && (
        <TopicForm draft={draft} setDraft={setDraft} onSave={onSave} onCancel={onCancel} />
      )}
      {filtered.length === 0 ? (
        <p className="text-[12px] text-text-secondary py-4 text-center bg-surface-2 rounded-xl">No topics here yet.</p>
      ) : (
        <div className="flex flex-col gap-2">
          {filtered.map((t) => <TopicRow key={t.id} topic={t} onUpdate={onUpdateTopic} onDelete={onDeleteTopic} />)}
        </div>
      )}
    </div>
  );
...
export default function TopicsModule({ topicLibrary, setTopicLibrary, onGoNext }) {
  ...
  function save() {
    ...
    setTopicLibrary((prev) => [...prev, { ...draft, id: crypto.randomUUID(), referenceCode }]);
    setAddingType(null);
  }
    ...
    reader.onload = (evt) => {
      ...
      if (topics.length) {
        setTopicLibrary((prev) => {
          let next = [...prev];
          topics.forEach((t) => {
            next = [...next, { ...t, referenceCode: nextReferenceCode(next, t.iroType, t.esrsTopicId) }];
          });
          return next;
        });
      }
      ...
      {topicLibrary.length > 0 && (
        <div className="rounded-2xl p-5 mt-3 flex items-center justify-between" style={{ background: 'linear-gradient(115deg, #3654D6, #2FA88A)' }}>
          <div>
            ...
            <p className="text-[12px]" style={{ color: 'rgba(245,246,250,0.9)' }}>Set up an assessment (quantitative or qualitative) to start rating these topics.</p>
          </div>
