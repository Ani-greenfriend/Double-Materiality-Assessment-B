...
async function exportChartsAsPdf(charts, filename) {
  const doc = new jsPDF({ orientation: 'landscape', unit: 'pt' });
  let first = true;
  for (const { svgEl, title } of charts) {
    if (!svgEl) continue;
    const blob = await svgToPngBlob(svgEl, 2.5);
...
export default function ResultsScreen({ iros, calibrations }) {
  ...
  const [downloadFormat, setDownloadFormat] = useState('csv'); // 'csv' | 'png' | 'pdf'
  ...
  const allTopics = topicIds.map((id) => ({ id, ta: aggregateTopic(id, iros), meta: ESRS_TOPICS.find((t) => t.id === id) }));
  ...
  const financialPoints = iros
    ...
    .map((iro) => {
      ...
      const likelihoods = iro.assessments.map((a) => a.financialLikelihood).filter((v) => v !== null && v !== undefined);
  ...
  async function handleDownload() {
    ...
    // PNG/PDF only cover the two heatmaps and the topic matrix — those are
    // the actual charts (SVG). The bar chart is a plain list of bars, not a
    // chart with axes to export as an image; CSV still covers it above.
    const charts = [];
    if (downloadSections.heatmaps) {
      charts.push({ svgEl: impactSvgRef.current, filename: 'impact-heatmap.png', title: 'Impact heatmap (Severity × Likelihood)' });
      charts.push({ svgEl: financialSvgRef.current, filename: 'financial-heatmap.png', title: 'Financial heatmap (Magnitude × Likelihood)' });
    }
    if (downloadSections.matrix) {
      charts.push({ svgEl: matrixSvgRef.current, filename: 'topic-matrix.png', title: 'Topic matrix' });
    }
    if (!charts.length) return;
    setDownloading(true);
    try {
      if (downloadFormat === 'png') await exportChartsAsPng(charts);
      else await exportChartsAsPdf(charts, 'apus-results-charts.pdf');
    } finally {
  ...
  return (
    <div>
      ...
      {/* ESRS TOPIC SUMMARY */}
      <p className="text-[13px] font-bold text-text-secondary tracking-wide mb-2">TOPIC SUMMARY — IROs ROLLED UP BY ESRS STANDARD</p>
      <div className="grid grid-cols-3 gap-2.5 mb-6">
        {allTopics.map((t) => {
          const color = PILLAR_COLOR[t.meta?.cat]?.text ?? '#8B8B98';
          const bg = PILLAR_COLOR[t.meta?.cat]?.bg ?? 'rgba(139,139,152,0.1)';
          const ratedCount = t.ta.iros.filter((i) => i.assessments.length > 0).length;
          return (
            <div key={t.id} className="rounded-xl p-3.5" style={{ background: bg, border: `1px solid ${color}44` }}>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[13px] font-bold" style={{ color }}>{t.meta?.name ?? t.id}</span>
                {t.ta.isMaterial && <span className="text-[9.5px] font-bold rounded-full px-2 py-0.5" style={{ background: '#D79A4C', color: '#100E15' }}>MATERIAL</span>}
              </div>
              <p className="text-[11.5px] text-text-secondary mb-1">{t.ta.iros.length} IRO{t.ta.iros.length === 1 ? '' : 's'} · {ratedCount} rated</p>
              <div className="flex gap-3 text-[11.5px]">
                <span>Impact <b className="text-text-primary">{t.ta.impactScore.toFixed(1)}</b></span>
                <span>Financial <b className="text-text-primary">{t.ta.financialScore.toFixed(1)}</b></span>
              </div>
            </div>
          );
        })}
      ...
      <div className="rounded-2xl p-5" style={{ background: 'linear-gradient(160deg, rgba(76,111,255,0.1), var(--color-surface))', border: '1px solid rgba(76,111,255,0.25)' }}>
        ...
        <div className="flex items-center gap-1.5 mb-4">
          ...
          {[['csv', 'Data (CSV)'], ['png', 'Image (PNG)'], ['pdf', 'Document (PDF)']].map(([val, label]) => (
            <button
        ...
        {downloadFormat !== 'csv' && (
          <p className="text-[11px] text-text-secondary mb-3">
            {downloadFormat === 'png' ? 'One PNG per selected chart.' : 'One PDF with each selected chart on its own page.'} The bar chart isn't included here — it's a plain list, not a chart with axes; use CSV for that one.
          </p>
        )}
        <button
